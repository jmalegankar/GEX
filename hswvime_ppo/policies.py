...
class HSWVIMEFeaturesExtractor(nn.Module):
    """
    HSWVIME features extractor (GRU/Mamba path).
    """

    def __init__(self, observation_space=None, *, wyner_dim: int, mu_dim: int):
        super().__init__()
        self._features_dim = 2 * mu_dim
    ...
    def forward(self, wyner_features: th.Tensor, mu_features: th.Tensor) -> th.Tensor:
        mu_features = mu_features.view(-1, 1, self.mu_dim)
        attn_output, _ = self.attn(mu_features, wyner_features, wyner_features, need_weights=False)
        x = th.cat((mu_features, attn_output), dim=-1).squeeze(1)
        return x

class HSWVIMEActorCriticPolicy(ActorCriticPolicy):
    """
    Policy class for actor-critic algorithms.
    Supports both GRU/Mamba Wyner (HSWVIMEFeaturesExtractor) and
    LMU Wyner (LMUActionFeatures) via automatic dispatch.
    """
    ...
    def __init__(
        ...
        features_extractor_class: type[nn.Module] = HSWVIMEFeaturesExtractor,
    ...
    def make_features_extractor(self):
        ...
        # Detect whether wyner is LMU-based (has .step method)
        self._use_lmu = hasattr(self.wyner_feature_extractor, 'step')
        return super().make_features_extractor()

    def _extract_features_lmu(
        self,
        s_tm1: th.Tensor,
        a_tm1: th.Tensor,
        s_t: th.Tensor,
        ...
        """LMU path: uses wyner.step() to get (h, m) and LMUActionFeatures."""
        with th.no_grad():
        ...
        # step() returns (h_t, m_t, packed_new_state)
        h_t, m_t, packed = self.wyner_feature_extractor.step(memory, mu)
        new_memory = packed.unsqueeze(1)  # (B, 1, packed_state_dim) for buffer storage

        # LMUActionFeatures.forward(h, m, mu)
        features = self.features_extractor(h_t, m_t, mu)
        return features, new_memory

    def _extract_features_gru(
        self,
        s_tm1: th.Tensor,
        a_tm1: th.Tensor,
        s_t: th.Tensor,
        ...
        """GRU/Mamba path: original HSWVIMEFeaturesExtractor."""
        with th.no_grad():
            s_tm1 = preprocess_obs(s_tm1, self.observation_space, normalize_images=self.normalize_images)
            s_t = preprocess_obs(s_t, self.observation_space, normalize_images=self.normalize_images)
            mu, _, skips = self.vae_feature_extractor.encode(s_tm1, a_tm1, s_t)
        ...
        # HSWVIMEFeaturesExtractor.forward(wyner_features, mu_features)
    ...
    def extract_features(
        ...
        if self._use_lmu:
            return self._extract_features_lmu(s_tm1, a_tm1, s_t, memory)
        else:
            return self._extract_features_gru(s_tm1, a_tm1, s_t, memory, timestep)
