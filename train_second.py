"""
LMU-PPO (colleague cell) on MiniGrid-Memory envs.

Run:
    # Chunked TBPTT (default):
    python train_second.py --env MemoryS7  --seed 0
    python train_second.py --env MemoryS13 --seed 0

    # Full episode BPTT on S7:
    python train_second.py --env MemoryS7 --seed 0 --full_bptt
"""
...
from lmu_ppo_second.lmu_ppo import LMUPPO
...
def main():
    ...
    print(f"\nLMU-PPO (colleague)  ·  {env_id}  ·  seed={args.seed}  ·  {bptt_mode}")
    ...
    model.learn(
        ...
        callback=eval_cb,
        tb_log_name=f"lmu_second_{args.env}_{'full_bptt' if args.full_bptt else 'tbptt'}_s{args.seed}",
        progress_bar=True,
    )

    save_path = f"lmu_ppo_second_{args.env}_s{args.seed}"
