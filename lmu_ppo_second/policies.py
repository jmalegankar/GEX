...
from lmu_colleague import LMUCell as ColleagueLMUCell
...
class LMUActorCriticPolicy(nn.Module):
    ...
    def __init__(
        ...
        self.lmu_cell = ColleagueLMUCell(
    ...
    def _critic_input(self, h: torch.Tensor, m: torch.Tensor) -> torch.Tensor:
        """
        h : (B, n, C)   ← colleague cell returns 3D h
        m : (B, d, C)
        → (B, n + C)
        """
        h_pooled = h.mean(dim=-1)         # (B, n, C) → (B, n), pool over channels
        m_pooled = m.mean(dim=1)          # (B, d, C) → (B, C), pool over Legendre dim
        return torch.cat([h_pooled, m_pooled], dim=-1)
    ...
    def evaluate_actions(
        ...
        for k in range(K):
            ...
            h = h * (1.0 - reset.unsqueeze(-1))              # (B, n, C)
