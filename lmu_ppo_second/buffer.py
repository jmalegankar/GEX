...
class LMURolloutBufferSamples(NamedTuple):
    ...
    lmu_h:         th.Tensor             # (B, hidden_size, num_channels) — chunk-start state
...
class LMURolloutBuffer(DictRolloutBuffer):
    ...
    lmu_h: np.ndarray   # (T, n_envs, hidden_size, encoder_dim)
    ...
    def reset(self) -> None:
        self.lmu_h = np.zeros(
            (self.buffer_size, self.n_envs, self.hidden_size, self.encoder_dim),
            dtype=np.float32,
        )
        self.lmu_m = np.zeros(
    ...
    def add(
        ...
        value:         th.Tensor,
        log_prob:      th.Tensor,
        lmu_h:         th.Tensor,   # (n_envs, hidden_size, encoder_dim)
