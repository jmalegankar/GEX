...
def _run_diagnostic(label, task, cell_type, config_overrides, expected_solved_lift):
    with warnings.catch_warnings():
        ...
        try:
            ...
            print("=" * 70)
            print(f"  task:    {task}  cell: {cell_type}")
            ...
            if lift > solved_threshold:
                interp = "solved"
                print(f"    SOLVED (lift {lift:+.3f} > {solved_threshold:+.2f}).")
                print(f"    LayerNorm WAS the missing piece.")
            elif lift > 0.10:
                interp = "partial"
                print(f"    PARTIAL (lift {lift:+.3f}).")
            elif chi2['consistent_with_uniform']:
                interp = "no_learning"
                print(f"    NO LEARNING — uniform actions.")
            else:
                interp = "marginal_prior"
                print(f"    MARGINAL-PRIOR ONLY (lift {lift:+.3f}, chi2 {chi2['chi2']:.1f}).")
...
def test_H_gru_easy_with_layernorm():
    """
    GRU + RepeatPreviousEasy + head_layernorm=True.
    Cleanest test: easiest task, simplest cell, only head_layernorm change.
    Compare to D's lift=+0.056 / chi2=6.2 / no_learning.
    """
    result = _run_diagnostic(
        label="H: GRU + Easy + head_layernorm=True",
        task="popgym-RepeatPreviousEasy-v0",
        cell_type="gru",
        config_overrides={"head_layernorm": True},
        expected_solved_lift=0.7,
    )
    print(f"\n  H summary: lift={result['lift']:+.3f}  chi2={result['chi2']:.1f}  "
          f"interp={result['interp']}")
    print(f"  D reference (no layernorm): lift=+0.056  chi2=6.2  interp=no_learning")